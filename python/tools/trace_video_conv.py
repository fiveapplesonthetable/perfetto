...
def resolve_region(tp, clip):
  ...
  ends = [r.ts + (r.dur if getattr(r, 'dur', None) is not None else 0)
          for r in rows]
...
def load_stream(bin_path, trace, clip):
  ...
  if not has_nal(stream, SPS_NAL_TYPE):
    print(f'warning: {trace} has no SPS (H.264 params); ffmpeg may not be able '
          'to decode it.', file=sys.stderr)
...
def mux(stream, fps, out_path):
  ...
  try:
    ...
    run_ffmpeg(['-r', str(fps), '-f', 'h264', '-i', raw, '-c', 'copy',
...
def mux_compare(left, right, titles, out_path):
  ...
  files = [write_temp(left[0], '.h264'), write_temp(right[0], '.h264'),
           write_temp(titles[0], '.txt'), write_temp(titles[1], '.txt')]
  ...
  try:
    ...
    run_ffmpeg(['-r', str(left[1]), '-f', 'h264', '-i', raw_l,
                '-r', str(right[1]), '-f', 'h264', '-i', raw_r,
...
def parse_args():
  ...
  ap.add_argument('--trace-processor', metavar='PATH',
  ...
  g.add_argument('--display-id', type=int, help='which video stream (see --list)')
  g.add_argument('--start', type=int, help='clip start (trace ts, ns)')
  g.add_argument('--end', type=int, help='clip end (trace ts, ns)')
  g.add_argument('--query', help='clip to the region a SQL query selects '
  ...
  g2.add_argument('--compare', metavar='TRACE',
                  help='second trace; its video is placed to the right')
  g2.add_argument('--display-id2', type=int, help='stream to use from --compare')
  g2.add_argument('--start2', type=int, help='clip start for --compare (ts, ns)')
...
def main():
  ...
  if args.compare:
    ...
    titles = (args.title or os.path.basename(args.trace),
              args.title2 or os.path.basename(args.compare))
