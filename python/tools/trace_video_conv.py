...
def mux(cfg_frames, sel, start_ts, end_ts, codec_string, speed, out_path):
  ...
  n = len(sel)
  gaps = [sel[i + 1].ts - sel[i].ts for i in range(n - 1)
...
def fill_card(video, lead, content_end, total, out_path):
  ...
  filters = fill_filters(find_font(), dims[1] if dims else 720, lead,
                         content_end, total)
  run_ffmpeg(['-i', video, '-vf', ','.join(filters), '-c:v', 'libx264',
...
def clip_span(clip, mp4, speed):
  ...
  mux(cfg, sel, None if lead else start, None if tail_gap else end, codec, speed,
      mp4)
...
def main():
  ...
  if args.compare:
    ...
    print(f'Wrote {args.output}: {len(left.sel)} + {len(right.sel)} frames side '
