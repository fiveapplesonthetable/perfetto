...
class AndroidParser(TestSuite):
  ...
  def test_android_framework_track_event_process(self):
    return DiffTestBlueprint(
        ...
        query="""
        SELECT p.pid, p.name, p.uid, p.package_uid, p.defining_uid,
               t.fw_start_ts, t.fw_end_ts
        FROM __intrinsic_android_track_event_process t
        JOIN process p USING (upid);
        """,
        out=Csv("""
          "pid","name","uid","package_uid","defining_uid","fw_start_ts","fw_end_ts"
          100,"com.example.app",10001,10123,10456,2000,5000
        """))
