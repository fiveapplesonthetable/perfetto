...
class AndroidParser(TestSuite):
  ...
  def test_android_process_state_metadata(self):
    return DiffTestBlueprint(
        ...
        out=Csv("""
          ...
          200,"com.example.running",20001,0,10,50000000,"[NULL]"
          300,"system_server",1000,0,"[NULL]",10000000,"[NULL]"
        """))

  def test_android_framework_binder_died_recycled_flush(self):
    return DiffTestBlueprint(
        trace=Path('android_framework_binder_died_recycled_flush.textproto'),
        query="""
        SELECT
          p.pid,
          p.name,
  ...
  def test_android_framework_binder_died_pretrace(self):
    return DiffTestBlueprint(
        trace=Path('android_framework_binder_died_pretrace.textproto'),
        query="""
        SELECT
          p.pid,
          p.name,
          ...
          p.start_ts,
          p.end_ts,
          t.end_ts AS main_thread_end_ts
        FROM process p
        LEFT JOIN __intrinsic_android_track_event_process s
          ON s.upid = p.upid
        LEFT JOIN thread t
          ON t.upid = p.upid AND t.is_main_thread
        WHERE p.pid > 0
        ORDER BY p.pid, p.start_ts;
        """,
        out=Csv("""
          "pid","name","uid","start_seq_id","start_ts","end_ts","main_thread_end_ts"
          100,"com.example.appa",10001,1,"[NULL]",4000000000,4000000000
          100,"com.example.appb",10002,2,5000000000,"[NULL]","[NULL]"
        """))

  def test_android_framework_ftrace_process_start(self):
    return DiffTestBlueprint(
        trace=Path('android_framework_ftrace_process_start.textproto'),
        query="""
        SELECT
          p.pid,
          p.name,
        ...
        """,
        out=Csv("""
          "pid","name","uid","start_seq_id","start_ts","end_ts","fw_end_ts"
          100,"zygote64",10001,1,900000000,4000000000,6000000000
          100,"zygote64",10002,2,4500000000,"[NULL]","[NULL]"
        """))

  def test_android_process_metadata_lifecycle(self):
    return DiffTestBlueprint(
        trace=Path('android_process_metadata_lifecycle.textproto'),
        query="""
        SELECT
          p.pid,
          p.name,
          ...
          p.end_ts,
          t.end_ts AS main_thread_end_ts,
          (SELECT COUNT(*) FROM __intrinsic_android_process_state s
           WHERE s.upid = p.upid AND s.is_initial = 0) AS state_changes
        FROM process p
        LEFT JOIN thread t ON t.upid = p.upid AND t.is_main_thread
        WHERE p.pid > 0
        ORDER BY p.pid, p.start_ts;
        """,
        out=Csv("""
          "pid","name","uid","start_ts","end_ts","main_thread_end_ts","state_changes"
          1,"com.example.app",10001,20000000,2000000000,2000000000,1
          100,"com.example.appa",10003,30000000,6000000000,6000000000,0
          100,"com.example.appb",10002,5000000000,"[NULL]","[NULL]",0
        """))
