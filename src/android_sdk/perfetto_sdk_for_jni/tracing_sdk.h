...
/**
 * @brief Represents a correlation id linking events that are part of the same
 * logical operation (TrackEvent's correlation_id / correlation_id_str).
 *
 * The value (integer or string) is set right before each emit, like DebugArg.
 * A string value is not owned: it must stay valid until the event is emitted.
 */
class CorrelationId {
 public:
