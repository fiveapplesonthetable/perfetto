...
public final class PerfettoTrack {
   ...
   * SIBLING_MERGE_BEHAVIOR_*} values. Returns a new handle; the original is
   * unchanged.
   */
   ...
   * string key (the two are a protobuf oneof). Returns a new handle; the
   ...
   * meaningful when {@link #setSiblingMergeBehavior} is {@link
   * #SIBLING_MERGE_BEHAVIOR_BY_SIBLING_MERGE_KEY}. Clears any previously set
   * integer key (the two are a protobuf oneof). Returns a new handle; the
   * original is unchanged.
   */
