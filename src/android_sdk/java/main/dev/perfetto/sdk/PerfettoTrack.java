 ...
 * the C SDK's nested-track behaviour. Each track uuid is derived natively exactly
 * as the C SDK derives it; nothing is hardcoded in Java.
 *
 * <p>This is the nesting-only shape supported by the high-level ABI; sibling
 * ordering, counter units and similar are intentionally not exposed here. This is
 * the single named/nested track primitive: the flat {@code usingProcessNamedTrack}
 * / {@code usingThreadNamedTrack} builder helpers are sugar over a one-level
 * {@code PerfettoTrack}.
 */
public final class PerfettoTrack {
  // Root scope of the chain. Kept in sync with the native mapping in
  // NestedTracks (tracing_sdk.cc).
  ...
  // emitting thread (the common case, and the only shape child()/process()/
  // thread()/global() produce). Set explicitly only by the thread-scoped flat
  // helper, which accepts an arbitrary tid.
  final long mTid;
  ...
  private PerfettoTrack(
    ...
    mCacheKey = computeCacheKey(rootType, tid, names, ids, isNameStatic, isCounter);
  }

  // A PerfettoTrack is a value: two handles describing the same chain are equal.
  // The flat-track sugar (usingProcessNamedTrack etc.) builds a fresh handle per
  // emit, so the builder's track cache matches on content, not identity, to reuse
  // the cached native NestedTracks. hashCode() returns the precomputed cache key,
  // derived from the same fields equals() compares.
  @Override
  public boolean equals(Object obj) {
    if (this == obj) {
      return true;
    }
    if (!(obj instanceof PerfettoTrack)) {
      return false;
    }
    PerfettoTrack o = (PerfettoTrack) obj;
    return mRootType == o.mRootType
        && mTid == o.mTid
        && Arrays.equals(mNames, o.mNames)
        && Arrays.equals(mIds, o.mIds)
        && Arrays.equals(mIsNameStatic, o.mIsNameStatic)
        && Arrays.equals(mIsCounter, o.mIsCounter);
  }
  ...
  public PerfettoTrack childCounter(@CompileTimeConstant String name) {
    return appendLevel(0, name, /* isNameStatic= */ true, /* isCounter= */ true);
  }

  // --- Internal factories used by the flat named-track builder helpers, which
  // make those helpers sugar over a one-level PerfettoTrack. These take the
  // leaf id and name-static flag the public API doesn't, and (for the thread
  // helper) an explicit tid. ---

  /** A one-level (flat) track. Used by the zero-alloc sugar path on a cache miss. */
  ...
  /**
   * The cache key for a one-level track, computed without building a handle so
   * the flat-track sugar can do its cache lookup allocation-free. Must match
   * {@link #computeCacheKey} for a single-element chain.
   */
  ...
  private static PerfettoTrack rooted(int rootType, long tid, String name) {
    return new PerfettoTrack(
        rootType, tid, new String[] {name}, new long[] {0}, new boolean[] {true},
        new boolean[] {false});
  }
  ...
  private static int computeCacheKey(
    ...
    for (boolean s : isNameStatic) {
      h = 31 * h + (s ? 1 : 0);
    }
    for (boolean c : isCounter) {
      h = 31 * h + (c ? 1 : 0);
    }
    return h;
  }
