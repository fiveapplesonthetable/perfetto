...
class ZstdCompressor {
 public:
  ...
  // buffer (caller frees; the frame length is written to |*out_size|) or nullptr
  // on error or when zstd is not compiled in.
  static std::unique_ptr<uint8_t, base::FreeDeleter> CompressFully(
      const uint8_t* data,
      size_t len,
      size_t* out_size,
      int level);
