...
class ProtoToArgsParser {
 ...
 private:
  ...
  // Descriptor index of the (flags_enum) enum on |field| (cached), or nullopt
  // if unannotated.
  std::optional<uint32_t> ResolveFlagsEnum(const FieldDescriptor& field);

  // Emits one string arg per set flag under the field's key; unmatched bits
  // become a trailing hex element.
  ...
  // Caches (flags_enum) resolution: enum full name -> its descriptor index.
  base::FlatHashMap<std::string, std::optional<uint32_t>>
      flags_enum_idx_by_name_;
