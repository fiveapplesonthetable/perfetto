...
class StorageTablesPlugin : public Plugin<StorageTablesPlugin> {
 public:
  ...
  void RegisterDataframes(std::vector<PluginDataframe>& out) override {
    ...
    AddDataframe(out, s->mutable_process_state_mutation_event_table());
    AddDataframe(out, s->mutable_process_state_adj_compute_table());
    AddDataframe(out, s->mutable_process_state_adj_step_table());
