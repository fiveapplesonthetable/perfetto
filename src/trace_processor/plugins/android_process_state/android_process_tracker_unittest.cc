...
TEST_F(AndroidProcessTrackerTest, StartAdoptsOwnerWithoutSeqId) {
  UniquePid live = context_.process_tracker->GetOrCreateProcess(10);
  EXPECT_EQ(tracker_->GetOrStartProcess(100, 10, 5, Str("app")), live);
  EXPECT_EQ(tracker_->GetStartSeqId(live), 5);
}

TEST_F(AndroidProcessTrackerTest, PreTraceOwnerEndedByItsDeath) {
  // Running before the trace: no start, no seq id.
  ...
  EXPECT_EQ(MainThreadEnd(old), 300);
  EXPECT_EQ(tracker_->GetStartSeqId(old), 1);

  // The pid is free again, so the next start is a new process.
  UniquePid b = tracker_->GetOrStartProcess(400, 10, 2, Str("b"));
  EXPECT_NE(b, old);
  EXPECT_FALSE(Process(b).end_ts().has_value());
}

TEST_F(AndroidProcessTrackerTest, DeathAfterReuseOfPreTracePidEndsNothing) {
  // The owner has no seq id, so the start adopts it; a later death for
  // another seq id cannot be attributed and must not end the owner.
  UniquePid owner = context_.process_tracker->GetOrCreateProcess(10);
  ASSERT_EQ(tracker_->GetOrStartProcess(200, 10, 2, Str("b")), owner);
  EXPECT_FALSE(tracker_->EndProcess(300, 10, 1).has_value());
  EXPECT_FALSE(Process(owner).end_ts().has_value());
}
