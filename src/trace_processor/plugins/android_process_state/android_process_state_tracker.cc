...
void AndroidProcessStateTracker::SetFrameworkIsProcessAuthority() {
...
void AndroidProcessStateTracker::ParseProcessStateDump(
  ...
  for (auto it = dump.record(); it; ++it) {
    ...
    std::optional<UniquePid> opt_upid;
    // Without ftrace nothing else reports these processes, so the dump
    // creates the ones the trace has not seen.
