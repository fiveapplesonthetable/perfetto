 ...
 private:
  void SetProcessMetadata(UniquePid upid, protozero::ConstBytes process_start) {
    ...
    if (evt.has_package_uid()) {
      (*trace_context_->storage->mutable_process_table())[upid].set_package_uid(
          static_cast<uint32_t>(evt.package_uid()));
    }
    if (evt.has_defining_uid()) {
      (*trace_context_->storage->mutable_process_table())[upid]
          .set_defining_uid(static_cast<uint32_t>(evt.defining_uid()));
    }
