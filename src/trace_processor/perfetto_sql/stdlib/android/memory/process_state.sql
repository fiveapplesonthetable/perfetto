...
WITH latest_b AS (
  SELECT
    b.*,
    ROW_NUMBER() OVER (
      PARTITION BY binding_id ORDER BY snapshot_id DESC
    ) AS rn
  FROM android_process_state_binding b
  WHERE b.snapshot_id <= $snap_id
),
latest_s AS (
  SELECT
    s.*,
    ROW_NUMBER() OVER (
      PARTITION BY service_id ORDER BY snapshot_id DESC
    ) AS rn
  FROM android_process_state_service s
  WHERE s.snapshot_id <= $snap_id
)
SELECT
  b.client_pid,
