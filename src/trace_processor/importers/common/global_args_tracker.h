...
class GlobalArgsTracker {
 public:
  ...
  struct CompactArg {
    ...
    StringId name_key = kNullStringId;
  };
 ...
 private:
  ...
  ArgSetId AddArgSet(const void* start, const void* end, uint32_t stride) {
    ...
    for (const CompactArg* ptr : valid) {
      ...
      // An annotation is a property of the key, so record each (key, annotation)
      // once rather than per arg set.
      if (!arg.annotation.is_null()) {
        ...
        if (seen_annotations_.Insert(dedup, true).second) {
          ...
          annotation.name_key = arg.name_key;
