...
ProcessStateModule::ProcessStateModule(ProtoImporterModuleContext* module_context,
...
void ProcessStateModule::ParseSnapshot(int64_t ts, ConstBytes blob) {
  ...
  for (auto it = snap.interned_strings(); it; ++it) {
    interned.push_back(context_->storage->InternString((*it).ToStdStringView()));
  }
  ...
  if (snap.has_global_state()) {
    ...
    snap_row.last_memory_level_normal = global.last_memory_level_normal() ? 1 : 0;
  ...
  auto snap_id =
      context_->storage->mutable_process_state_snapshot_table()->Insert(snap_row).id;
  ...
  for (auto it = snap.process(); it; ++it) {
    ...
    if (pr.has_adj_type_idx()) {
      ...
      if (!s.is_null()) prow.adj_type = s;
    }
    if (pr.has_adj_source_pid()) prow.adj_source_pid = pr.adj_source_pid();
    if (pr.has_adj_target_pid()) prow.adj_target_pid = pr.adj_target_pid();
    if (pr.has_adj_source_proc_state()) {
      prow.adj_source_proc_state = pr.adj_source_proc_state();
    }
    if (pr.has_rep_proc_state()) prow.rep_proc_state = pr.rep_proc_state();
    if (pr.has_has_recent_tasks()) {
      prow.has_recent_tasks = pr.has_recent_tasks() ? 1 : 0;
    }
    if (pr.has_is_frozen()) prow.is_frozen = pr.is_frozen() ? 1 : 0;
    if (pr.has_cached_adj()) prow.cached_adj = pr.cached_adj();
  ...
  for (auto it = snap.binding(); it; ++it) {
    ...
    if (b.has_client_process_name_idx()) {
      ...
      if (!s.is_null()) brow.client_process_name = s;
    }
  ...
  for (auto it = snap.provider(); it; ++it) {
    ...
    if (cpr.has_class_name_idx()) {
      ...
      if (!s.is_null()) prow.class_name = s;
    }
