...
PROCESS_STATE_SNAPSHOT_TABLE = Table(
    ...
    tabledoc=TableDoc(
        ...
        group='Android',
        columns={
            'ts': 'Boot-clock timestamp the snapshot was observed.',
            'oom_adj_reason':
                'OomAdjReason that triggered this snapshot. 0 == NONE.',
            'top_pid': 'PID of the top process at this moment, if any.',
            'is_full':
...
PROCESS_STATE_PROCESS_TABLE = Table(
    ...
    tabledoc=TableDoc(
        ...
        columns={
            ...
            'pid': 'Process id.',
            'uid': 'Owning UID.',
            'user_id': 'Android multi-user user id.',
            'process_name': 'ProcessRecord.processName.',
            'package_name': 'Owning ApplicationInfo.packageName.',
            'lru_index': 'Position in mLruProcesses (0 = MRU).',
            'cur_adj': 'ProcessRecord.mState.curAdj.',
            'cur_raw_adj': 'curRawAdj — pre-clamp adj.',
            'set_adj': 'setAdj — last adj written to lmkd.',
            'max_adj': 'Maximum adj this process is allowed to take.',
            'cur_proc_state': 'ActivityManager.PROCESS_STATE_*.',
            'set_proc_state': 'Last proc state reported.',
            'cur_raw_proc_state': 'Pre-clamp proc state.',
            'cur_capability': 'PROCESS_CAPABILITY_* bitmask.',
            'set_capability': 'Last capability reported.',
            'cur_sched_group': 'ProcessList.SCHED_GROUP_*.',
            'set_sched_group': 'Last sched group applied.',
            'has_foreground_activities': '1 if any foreground activities.',
            'has_top_ui': '1 if process has top UI.',
            'has_overlay_ui': '1 if process has an overlay UI window.',
            'has_shown_ui': '1 if process has at some point shown UI.',
            'has_visible_activities': '1 if any visible activities.',
            'has_started_services': '1 if any services were startService()ed.',
            'persistent': '1 if process is marked persistent.',
            'isolated': '1 if process is isolated.',
        ...
        columns={
            'snapshot_id':
                ColumnDoc(
                    ...
                    joinable='android_process_state_snapshot.id'),
            'uid': 'Android UID.',
            'cur_proc_state': 'Aggregate cur proc state across processes.',
            'set_proc_state': 'Last reported proc state.',
            'cur_capability': 'Aggregate PROCESS_CAPABILITY_* bitmask.',
            'idle': '1 if UidRecord.isIdle().',
            'ephemeral': '1 if UidRecord.isEphemeral().',
        },
    ),
)
        ...
        columns={
            'snapshot_id':
                ColumnDoc(
                    doc='Foreign key into android_process_state_snapshot.',
                    joinable='android_process_state_snapshot.id'),
            'service_id': 'Service id, unique within (snapshot_id).',
            'owning_pid':
                'PID of the ProcessRecord this service runs in. 0 if absent.',
            'short_name': 'ComponentName.flattenToShortString.',
            'package_name': 'Package implementing the service component.',
            'is_foreground': '1 if startForeground() is in effect.',
            'foreground_id':
            ...
            'delayed': '1 if waiting to start in background.',
            'delayed_stop': '1 if stopped but in a delayed restart.',
            'execute_nesting':
                'Outstanding execute operations keeping foreground active.',
            'execute_fg':
                '1 if any of the executes were started from foreground.',
            'restart_count': 'Number of restarts performed in a row.',
            'crash_count':
...
PROCESS_STATE_BINDING_TABLE = Table(
    ...
    tabledoc=TableDoc(
        ...
        columns={
            ...
            'client_uid': 'UID of the client process.',
            'client_process_name': 'Process name of the client.',
            'service_id':
                ColumnDoc(
            ...
            'client_label': 'String resource labeling this client (or 0).',
            'service_dead':
...
PROCESS_STATE_PROVIDER_TABLE = Table(
    ...
    tabledoc=TableDoc(
        ...
        columns={
            ...
            'owning_pid': 'PID of the hosting ProcessRecord.',
            'authority':
                'Provider authority (e.g. com.android.contacts).',
            'package_name': 'Package implementing the provider.',
            'class_name': 'Class implementing the provider.',
...
PROCESS_STATE_PROVIDER_BINDING_TABLE = Table(
    ...
    tabledoc=TableDoc(
        ...
        columns={
            ...
            'dead': '1 if the connection is dead.',
            'waiting':
...
PROCESS_STATE_MUTATION_EVENT_TABLE = Table(
    ...
    tabledoc=TableDoc(
        ...
        columns={
            ...
            'prev_adj': 'Adj before the change.',
            'next_adj': 'Adj after the change.',
            'prev_proc_state': 'Proc state before.',
            'next_proc_state': 'Proc state after.',
            'prev_capability': 'Capability before.',
            'next_capability': 'Capability after.',
            'prev_sched_group': 'Sched group before.',
            'next_sched_group': 'Sched group after.',
            'oom_adj_reason': 'OomAdjReason if event is an OOM_ADJ_RECOMPUTE.',
            'bind_flags': 'BIND_* bitmask for SERVICE_BIND events.',
            'fg_service_types': 'FGS type bitmask for FGS_START / FGS_TYPE_CHANGE.',
            'label': 'Free-form label (action name, broadcast component, etc).',
        },
    ),
)
...
PROCESS_STATE_ADJ_COMPUTE_TABLE = Table(
    ...
    tabledoc=TableDoc(
        ...
        columns={
            ...
            'pid': 'PID being recomputed.',
            'start_ts':
                'Boot-clock start of the computation in nanoseconds.',
            'duration_ns': 'Duration of the computation in nanoseconds.',
            'oom_adj_reason': 'Reason that triggered this cycle.',
            'final_adj': 'Adj produced by this computation.',
            'final_proc_state': 'Proc state produced.',
            'final_capability': 'Capability bitmask produced.',
            'final_sched_group': 'Sched group produced.',
            'prev_adj': 'Adj before the computation.',
            'prev_proc_state': 'Proc state before.',
            'prev_capability': 'Capability before.',
            'prev_sched_group': 'Sched group before.',
        },
