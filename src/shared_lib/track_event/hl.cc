...
uint64_t EmitNamedTrack(uint64_t parent_uuid,
  ...
  if (incr_state->seen_track_uuids.insert(uuid).second) {
    ...
    // The merge key is a protobuf oneof: a string key, if any, wins over the
    // integer key. The integer key cannot be told apart from its zero default,
    // so it's only emitted when the behavior says it's meaningful.
    if (track.sibling_merge_key_str) {
      track_descriptor->set_sibling_merge_key(track.sibling_merge_key_str);
    } else if (track.sibling_merge_behavior ==
               PERFETTO_TE_HL_SIBLING_MERGE_BEHAVIOR_BY_SIBLING_MERGE_KEY) {
      track_descriptor->set_sibling_merge_key_int(track.sibling_merge_key_int);
    }
  }
  return uuid;
}
