...
export function buildSliceFlamegraphMetrics(nodesTable: string) {
...
async function buildSliceFlamegraphResources(
  trace: Trace,
  nodesSql: string,
  ...
  const metrics = buildSliceFlamegraphMetrics(nodes.name);
...
export class SliceFlamegraph implements m.ClassComponent<SliceFlamegraphAttrs> {
  ...
  view({attrs}: m.CVnode<SliceFlamegraphAttrs>): m.Children {
    const {isPending, data} = this.resourcesSlot.use({
      ...
      compute: () =>
        buildSliceFlamegraphResources(attrs.trace, attrs.nodesSql, this.queue),
    });
    if (data === undefined) {
