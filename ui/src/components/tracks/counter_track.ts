...
// How a track carrying several counters draws them in one plot: overlaid on
// one axis, or stacked on top of each other. Expanding the track instead gives
// each counter its own row.
export type CounterLayout = 'lines' | 'stacked';
...
export class CounterTrack implements TrackRenderer {
  ...
  getTrackShellButtons(): m.Children {
    ...
    return m(Button, {
      ...
      icon: this.rows ? Icons.UnfoldLess : Icons.UnfoldMore,
      tooltip: this.rows
        ? 'Show as one plot'
  ...
  private get view(): CounterLayout | 'rows' {
    return this.rows ? 'rows' : this.layout;
  }
  ...
  // The number of plots stacked vertically: one per shown counter in rows.
  private get plotCount(): number {
    if (this.isMultiSeries && this.view === 'rows') {
    ...
    switch (this.view) {
      ...
      case 'rows': {
        ...
        visible.forEach((series, i) => {
          this.drawRow(trackCtx, series, top + i * rowHeight, rowHeight);
        });
        break;
      }
      default:
