...
export class Flamegraph implements m.ClassComponent<FlamegraphAttrs> {
  ...
  private renderTooltip() {
    ...
    return m(
      ...
      Array.from(properties, ([_, property]) => {
        if (property.isVisible) {
          const num = Number(property.value);
          const text =
            property.unit !== undefined && Number.isFinite(num)
              ? displaySize(num, property.unit)
              : property.value;
          return m(
            '.tooltip-text-line',
            m('.tooltip-bold-text', property.displayName + ':'),
            m('.tooltip-text', text),
          );
        }
        return null;
...
// Resolve a `diff:<score>` hint into a ColorScheme. Scores are quantised to
// two decimals before caching: adjacent scores are visually identical, and it
// bounds the cache, which would otherwise grow by one entry per distinct
// float in every dataset.
function getColorSchemeFromHint(hint: string): ColorScheme {
  const score =
    Math.round(clampSigned(Number(hint.split(':')[1] ?? '0')) * 100) / 100;
  const key = `${score}`;
  const cached = hintSchemeCache.get(key);
  if (cached !== undefined) return cached;
  const base = pprofDiffColor(score);
  const scheme = makeColorScheme(base, base.darken(8));
  hintSchemeCache.set(key, scheme);
  return scheme;
}
