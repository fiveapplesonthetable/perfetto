...
function ClassesDiffView(): m.Component<ClassesDiffViewAttrs> {
  ...
  async function load(currentEngine: Engine, baselineEngine: Engine) {
    ...
    error = null;
    try {
      // Snapshot the filters before await so a mid-flight selection change
      // doesn't mismatch left/right sides — `isGenStillValid` aborts the
      // overall fetch but the snapshot keeps the two sides consistent.
