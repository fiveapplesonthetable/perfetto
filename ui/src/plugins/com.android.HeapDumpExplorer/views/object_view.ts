...
// Open the flamegraph focused on the given object's reference path. Routed
// through the session so flamegraph state has a single owner.
export type OpenFlamegraphOnInstancePath = (
  objId: number,
  isDominator: boolean,
) => void;
...
interface ObjectViewAttrs {
  ...
  readonly openFlamegraphOnInstancePath: OpenFlamegraphOnInstancePath;
        ...
        row.className
          ? m(
              'button',
              {
                class: 'pf-hde-link',
                title: isDominator
                  ? 'Open in Flamegraph focused on this dominator path'
                  : 'Open in Flamegraph focused on this shortest path',
                onclick: () =>
                  vnode.attrs.openFlamegraphOnInstancePath(row.id, isDominator),
              },
              'View in Flamegraph',
            )
          : null;
