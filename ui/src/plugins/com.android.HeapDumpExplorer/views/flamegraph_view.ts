...
function buildDiffMetric(
  ...
  return {
    ...
    statement,
    unaggregatableProperties: UNAGG_PROPS,
    // Only surface the stats for THIS metric's dimension (size or count).
    // The other dimension is hidden to keep the tooltip compact — switch
    // metrics to see the other one. SUM mergeAggregation gives the
    // cumulative (subtree-summed) signed Δ when nodes are merged.
    aggregatableProperties:
      valueColumn === 'self_size'
        ? [
            {
...
function buildHeapGraphDiffMetrics(
  ...
  for (const s of METRIC_SPECS) {
    ...
    // Skipping the current-width variant in diff mode — its data is
    // already reachable by clicking the "Current" radio in the top-bar
    // (which switches the page out of diff mode entirely), so keeping it
    // in the diff dropdown is duplicate UX.
  }
  return metrics;
}
...
function buildCrossTraceHeapGraphDiffMetrics(
  ...
  for (const s of METRIC_SPECS) {
    ...
    // Current-width variant intentionally skipped — see the same-trace
    // builder for rationale.
  }
  return metrics;
}
...
function renderDiffLegend(): m.Child {
...
const FlamegraphView: m.ClosureComponent<FlamegraphViewAttrs> = () => {
  ...
  return {
    view({attrs}) {
      ...
      const legend = attrs.baseline ? renderDiffLegend() : null;
