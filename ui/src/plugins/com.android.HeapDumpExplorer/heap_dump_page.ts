...
// A same-trace baseline from a different process makes deltas reflect
// differences between processes, not changes over time — easy to do
// unintentionally when the trace has no process names.
function renderCrossProcessWarning(
  active: queries.HeapDump,
  baseline: BaselineDumpRefLike | null,
  mode: string,
): m.Children {
  if (mode !== 'diff' || baseline === null) return null;
  if (baseline.trace.disposable !== false) return null;
  if (baseline.dump.upid === active.upid) return null;
  const label = (d: queries.HeapDump) =>
    d.processName ?? (d.pid !== null ? `pid ${d.pid}` : 'an unnamed process');
  return m(
    Callout,
    {icon: 'warning', intent: Intent.Warning, className: 'pf-hde-mb-4'},
    `The baseline dump is from a different process (${label(baseline.dump)}) ` +
      `than the current dump (${label(active)}). Deltas show the difference ` +
      `between the two processes, not changes over time.`,
  );
}

interface BaselineDumpRefLike {
  readonly trace: {readonly disposable?: boolean};
...
export class HeapDumpPage implements m.ClassComponent<HeapDumpPageAttrs> {
  ...
  view({attrs}: m.Vnode<HeapDumpPageAttrs>) {
    ...
    return m(
      ...
      {class: 'pf-hde-page'},
      topBar,
      renderCrossProcessWarning(active, baseline, mode),
      m(
        'main',
        {class: 'pf-hde-main'},
        m(Tabs, {
