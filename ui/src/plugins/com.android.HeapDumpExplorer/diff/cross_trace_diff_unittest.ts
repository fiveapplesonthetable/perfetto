...
describe('cross-trace flamegraph diff: name-path pairing', () => {
  ...
  test('pairing: base-only path becomes a removed node under its paired parent', () => {
    ...
    const gone = out.find((r) => r.name === 'Gone')!;
    expect(gone.c_self_size).toBe(0);
    expect(gone.b_self_size).toBe(100);
    expect(gone.delta_size).toBe(-100);
    // Attached under the current tree's Root, with an id past cur's range.
    expect(gone.parent_id).toBe(1);
    expect(gone.id).toBeGreaterThan(1);
  });

  test('pairing: removed subtree chains under its removed parent', () => {
    const cur: RawTreeRow[] = [node(1, null, 'Root', 0)];
    const base: RawTreeRow[] = [
      node(11, null, 'Root', 0),
      node(12, 11, 'GoneParent', 40),
      node(13, 12, 'GoneChild', 60),
    ];
    const out = pairTrees(cur, base);
    const parent = out.find((r) => r.name === 'GoneParent')!;
    const child = out.find((r) => r.name === 'GoneChild')!;
    expect(parent.parent_id).toBe(1);
    expect(child.parent_id).toBe(parent.id);
    expect(child.delta_size).toBe(-60);
  });
