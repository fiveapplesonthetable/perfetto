...
// the `joined` CTE of the same-trace SQL: every cur node appears once, and
// baseline-only paths are appended as removed nodes (current value 0) so
// mass that moved or disappeared renders green instead of vanishing.
export function pairTrees(
  ...
  // Baseline-only paths: one removed node per path, attached under the
  // paired parent when the parent path exists in the current tree, else
  // under its (also synthesized) baseline parent. Ids are offset past the
  // current tree's ids, which come from a different engine.
  const curIdByKey = new Map<string, number>();
  for (const c of cur) {
    const k = curKeys.get(c.id);
    if (k !== undefined && !curIdByKey.has(k)) curIdByKey.set(k, c.id);
  }
  let nextId = cur.reduce((m, c) => Math.max(m, c.id), 0) + 1;
  const removedByKey = new Map<
    string,
    {row: RawTreeRow; id: number; selfSize: number; selfCount: number}
  >();
  for (const b of base) {
    const k = baseKeys.get(b.id);
    if (k === undefined || curIdByKey.has(k)) continue;
    const prev = removedByKey.get(k);
    if (prev === undefined) {
      removedByKey.set(k, {
        row: b,
        id: nextId++,
        selfSize: b.self_size,
        selfCount: b.self_count,
      });
    } else {
      prev.selfSize += b.self_size;
      prev.selfCount += b.self_count;
  ...
  const baseParentKey = (row: RawTreeRow): string | undefined =>
    row.parent_id !== null ? baseKeys.get(row.parent_id) : undefined;
  for (const [k, r] of removedByKey) {
    let pathId = pathIds.get(k);
    if (pathId === undefined) {
      pathId = pathIds.size;
      pathIds.set(k, pathId);
    }
    const pk = baseParentKey(r.row);
    const parentId =
      pk === undefined
        ? null
        : (curIdByKey.get(pk) ?? removedByKey.get(pk)?.id ?? null);
    out.push({
      ...r.row,
      id: r.id,
      parent_id: parentId,
      c_self_size: 0,
      c_self_count: 0,
      b_self_size: r.selfSize,
      b_self_count: r.selfCount,
      delta_size: -r.selfSize,
      delta_count: -r.selfCount,
      is_new: 0,
      path_hash_stable: String(pathId),
    });
  }
