...
export async function getBitmapList(
  ...
  await requireDominatorTree(engine);
  const dumpData = await loadBitmapDumpData(engine, activeDump);
  // Pulls the per-Bitmap row from the `android.bitmaps` stdlib —
  // including the new mId / mSourceId / storage-type / sender columns —
  // and joins back to the existing heap-graph object/class/dominator
  // shape so we don't lose the retained-size / class-kind columns the
  // rest of the gallery view depends on.
  await engine.query(`INCLUDE PERFETTO MODULE android.bitmaps;`);
  const res = await engine.query(`
    SELECT
      o.id,
      c.name AS cls,
    ...
    LEFT JOIN heap_graph_object_data od ON o.object_data_id = od.id
    LEFT JOIN heap_graph_dominator_tree d ON d.id = o.id
    LEFT JOIN android_heap_graph_bitmaps b ON b.object_id = o.id
    WHERE o.reachable != 0
      AND ${dumpFilterSql(activeDump, 'o')}
      AND (c.name = 'android.graphics.Bitmap'
        OR c.deobfuscated_name = 'android.graphics.Bitmap')
