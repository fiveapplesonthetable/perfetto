...
export async function getBitmapList(
  ...
  await requireDominatorTree(engine);
  const dumpData = await loadBitmapDumpData(engine, activeDump);
  // Pulls bitmap_id / source_id / storage_type / sender-process columns
  // from the `android.memory.heap_graph.bitmap` stdlib module, joined to
  // the existing heap-graph object/class/dominator shape so the other
  // gallery columns (retained sizes / class kind / etc.) still populate.
  // `native_ptr` is sourced inline from heap_graph_primitive because the
  // stdlib table doesn't expose it but the BitmapDumpData byte[] lookup
  // (see `loadBitmapDumpData`) needs it to map Bitmap -> encoded buffer.
  await engine.query(
  ...
  const res = await engine.query(`
    SELECT
      ...
      od.value_string,
      c.kind AS class_kind,
      CAST(b.width AS INT) AS width,
      CAST(b.height AS INT) AS height,
      CAST(b.density AS INT) AS density,
      MAX(CASE WHEN f.field_name GLOB '*mNativePtr' THEN f.long_value END) AS native_ptr,
      b.bitmap_id,
      b.bitmap_storage_type,
      b.source_id,
      CAST(b.source_pid AS INT) AS source_pid,
