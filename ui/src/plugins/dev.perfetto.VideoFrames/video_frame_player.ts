...
export class VideoFramePlayer {
  ...
  async downloadRegion(start: bigint, end: bigint): Promise<void> {
    ...
    const trimStart =
      this.frames[seed].isKey && seed < lo
        ...
        : undefined;
    await this.downloadFrames(this.frames.slice(seed, hi), '-region', trimStart);
  }
  ...
  private async downloadFrames(
    ...
    let mp4 = await muxToMp4(this.codecString!, config, frames, width, height, 30);
    if (trimStartSec !== undefined && trimStartSec > 0) {
      mp4 = await trimMp4(mp4, trimStartSec);
    }
    await download({content: mp4, fileName: `${this.displayName}${suffix}.mp4`});
  }
