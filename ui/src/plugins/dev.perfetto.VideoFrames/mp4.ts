...
export async function muxToMp4(
  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    await source.add(
      ...
      i === 0
        ? {decoderConfig: {codec: codecString, codedWidth: width, codedHeight: height}}
        : undefined,
    );
  }
...
export async function trimMp4(
  ...
  const input = new Input({formats: ALL_FORMATS, source: new BufferSource(mp4)});
  const output = new Output({
    format: new Mp4OutputFormat({fastStart: 'in-memory'}),
    target: new BufferTarget(),
  });
  const trim = endSec !== undefined ? {start: startSec, end: endSec} : {start: startSec};
