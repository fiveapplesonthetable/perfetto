...
// Crossfilter explorer: a DataGrid of the profiles plus a chart per column — a
// brushable histogram (numeric) or bar chart (categorical) sharing the grid's
// Filter[] — driving a merged flamegraph.
export class AggregateProfilesMergePage implements m.ClassComponent<AggregateProfilesMergePageAttrs> {
  ...
  private renderChart(
    ...
    return m(
      ...
      m(
        ...
        col.title,
        // Profiles with no value for this column go in the title, not a NULL bar.
        missing > 0 &&
          m('span.pf-aggregate-merge__chart-na', ` · ${missing} n/a`),
      ),
      col.kind === 'categorical'
        ? this.renderBar(attrs, filters, col, base)
  ...
  private renderBar(
    ...
    return m(BarChart, {
      ...
      selection: inValuesOf(filters, col.field).map((v) => String(v)),
      onBrush: (labels: Array<string | number>) => {
        const vals = labels.map((l) => String(l));
        this.setFilters(
          attrs,
          replaceField(
            filters,
            ...
            vals.length === 0
              ? []
              : [{field: col.field, op: 'in', value: vals}],
          ),
        );
      },
    });
  ...
  private rebuildFlamegraph(attrs: AggregateProfilesMergePageAttrs): void {
    ...
    for (const r of rows) {
      ...
      for (const st of attrs.sampleTypes) {
        ...
        if (met) {
          metrics.push(aggregateProfileMetric(st.key, st.unit, [met.aggId]));
        }
      }
...
function inValuesOf(filters: ReadonlyArray<Filter>, field: string): SqlValue[] {
  const f = filters.find((x) => x.field === field && x.op === 'in');
  return f && f.op === 'in' ? [...f.value] : [];
}
