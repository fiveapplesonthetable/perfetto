...
import {Button, ButtonBar, ButtonGroup, ButtonVariant} from '../../widgets/button';
...
export class TimeseriesView
  implements m.ClassComponent<TimeseriesViewAttrs>
{
  ...
  private renderLegendResize(model: ViewerModel) {
    return m('.pf-tsv__legend-resize', {
      ...
      onpointerdown: (e: PointerEvent) => {
        ...
        this.legendDrag = {startY: e.clientY, startH: model.legendHeight ?? DEFAULT_LEGEND_H};
        e.preventDefault();
      },
      onpointermove: (e: PointerEvent) => {
        if (this.legendDrag === undefined) return;
        const dy = e.clientY - this.legendDrag.startY;
        model.setLegendHeight(Math.max(28, Math.min(this.legendDrag.startH + dy, 480)));
      },
  ...
  private renderToolbar(model: ViewerModel) {
    ...
    return m(
      ...
      divider(),
      seg([
        {label: 'Stacked', active: model.layout === 'stacked', onclick: () => model.setLayout('stacked')},
        {label: 'Overlay', active: model.layout === 'overlay', onclick: () => model.setLayout('overlay')},
      ]),
      ...
      model.layout === 'overlay' &&
        m(Button, {
          ...
          compact: true,
          title: 'Stack the counters as a cumulative area chart (shared value axis)',
          onclick: () => model.setOverlayStacked(!model.overlayStacked),
        }),
      divider(),
      seg([
        {label: 'Value', active: model.yMode === 'value', onclick: () => void model.setYMode('value').then(m.redraw)},
        {label: 'Delta', active: model.yMode === 'delta', onclick: () => void model.setYMode('delta').then(m.redraw)},
        {label: 'Rate', active: model.yMode === 'rate', onclick: () => void model.setYMode('rate').then(m.redraw)},
      ]),
      divider(),
      m(Button, {
        icon: 'zoom_out_map',
      ...
      m(
        ...
        m('.pf-tsv__style-menu', [
          ...
          m('.pf-tsv__style-row', [
            ...
            seg([
              {label: 'Global', active: model.yRangeMode === 'global', title: "Locked to each counter's whole-trace range", onclick: () => model.setYRangeMode('global')},
              {label: 'Shared', active: model.yRangeMode === 'shared', title: 'One shared axis across all lanes, so heights are comparable', onclick: () => model.setYRangeMode('shared')},
              {label: 'Fit', active: model.yRangeMode === 'fit', title: 'Fits the samples currently in view', onclick: () => model.setYRangeMode('fit')},
            ]),
          ]),
          m('.pf-tsv__style-row', [
            m('span.pf-tsv__style-label', 'Scale'),
            seg([
              {label: 'Linear', active: model.yScale === 'linear', onclick: () => model.setYScale('linear')},
              {label: 'Log', active: model.yScale === 'log', onclick: () => model.setYScale('log')},
            ]),
          ]),
          model.layout === 'stacked' &&
            m('.pf-tsv__style-row', [
              ...
              seg([
                {label: 'S', active: model.laneHeight === 64, onclick: () => model.setLaneHeight(64)},
                {label: 'M', active: model.laneHeight === 96, onclick: () => model.setLaneHeight(96)},
                {label: 'L', active: model.laneHeight === 140, onclick: () => model.setLaneHeight(140)},
              ]),
            ]),
          ...
          model.layout === 'overlay' &&
            ...
            m('.pf-tsv__style-row', [
              ...
              seg([
                {label: 'All', active: model.overlayUnitFilter === undefined, onclick: () => model.setOverlayUnitFilter(undefined)},
  ...
  private renderLegend(model: ViewerModel) {
    ...
    return m(
      ...
      model.selected.slice(0, LEGEND_MAX).map((s, i) =>
        m(
          ...
          m(
            ...
            {onclick: () => model.toggleHidden(s), title: 'Click to show/hide this counter'},
          ...
          m(
            ...
            {
              position: PopupPosition.Bottom,
              trigger: m(Button, {icon: 'tune', title: 'Line style & colour', compact: true}),
            },
      ...
      extra > 0 && m('span.pf-tsv__legend-more', `+${extra.toLocaleString()} more`),
    );
  }
      ...
      m('.pf-tsv__style-row', [
        ...
        m('.pf-tsv__colors', [
          ...
          m(
            ...
            {title: 'Default colour', onclick: () => model.setColor(s, undefined)},
            'reset',
          ),
        ]),
      ]),
  ...
  private renderPicker(model: ViewerModel) {
    ...
    return m('.pf-tsv__picker', [
      ...
      m('.pf-tsv__picker-apply', [
        m('span.pf-tsv__picker-count', `${pending.size.toLocaleString()} selected`),
        m(Button, {
          label: 'Apply',
          variant: ButtonVariant.Filled,
          disabled: !dirty,
        ...
        m(Button, {label: 'Cancel', dismissPopup: true, onclick: () => (this.pending = undefined)}),
      ]),
    ]);
  }
}
