...
import {type AxisRange, logRange} from '../../components/widgets/charts_svg/common';
...
import {type CounterSeries, formatMetric, type LineStyle, type SeriesWindow, swatchClassFor} from './counter_index';
...
function niceValueTicks(rawMin: number, rawMax: number, approx: number): AxisRange {
...
export class TimeseriesChart
  implements m.ClassComponent<TimeseriesChartAttrs>
{
  ...
  private pinch?: {startDist: number; startView: {s: time; e: time}; centerX: number};
  ...
  oncreate(vnode: m.CVnodeDOM<TimeseriesChartAttrs>) {
    ...
    this.removeInvalidateListener = this.model.addInvalidateListener(() => this.wake());

    // WASD navigation, matching the main timeline: A/D pan, W/S zoom at cursor.
    this.wasd = new WasdNavigation({
      element: this.canvas,
      onPanned: (px) => {
        this.model.panBy((px / this.plotWidth()) * Number(this.model.viewDuration));
        this.wake();
      },
  ...
  private fetchList(): {series: CounterSeries[]; bpp: number} {
    ...
    if (model.layout === 'overlay') {
      ...
      const bpp = Math.max(
        0.25,
        Math.min(BUCKETS_PER_PX, TARGET_TOTAL_POINTS / (Math.max(1, series.length) * this.plotWidth())),
      );
      return {series, bpp};
    }
  ...
  private trackShellWidth(): number {
    const v = getComputedStyle(document.body).getPropertyValue('--track-shell-width');
  ...
  private draw() {
    ...
    this.sharedRange =
      this.model.yRangeMode === 'shared' ? this.computeSharedRange() : undefined;
    ...
    const px1s = this.ts.timeToPx(Time.fromRaw(this.model.traceStart + 1_000_000_000n));
    ...
    if (n > 0) {
      ...
      this.setScrollHeight(contentH > viewportH ? TOP_AXIS + contentH + BOTTOM : 0);
  ...
  private overlayY(
    ...
    return this.mapY(v, {min: r.min, max: r.max, ticks: []}, plotTop, plotH, this.model.yScale === 'log');
  }

  // Common unit across the visible overlay series, or undefined if they differ
  // (then the shared value axis shows plain numbers).
  private overlayUnit(visible: ReadonlyArray<{s: CounterSeries; i: number}>): string | undefined {
  ...
  private overlayAbsRange(
    ...
    if (this.model.yScale === 'log' && c.max > 0) {
      return thinTicks(logDrawRange(c.min > 0 ? c.min : c.max / 1e3, c.max), approx + 1);
    }
  ...
  private drawOverlayNormalized(
    ...
    const fracs =
      height < 150 ? [0, 0.5, 1] : height < 300 ? [0, 0.25, 0.5, 0.75, 1] : [0, 0.2, 0.4, 0.6, 0.8, 1];
    ctx.font = AXIS_FONT;
    ctx.textAlign = 'right';
    ctx.textBaseline = 'middle';
  ...
  private drawOverlayAbsolute(
    ...
    this.drawOverlayValueAxis(pal, top, bottom, range, this.overlayUnit(visible), log);
    const plotTop = top + LANE_PAD_TOP;
    const plotH = Math.max(1, bottom - top - LANE_PAD_TOP - LANE_PAD_BOTTOM);
  ...
  private drawOverlayLines(
    ...
    for (const {s: series, i} of visible) {
      ...
      for (let k = 0; k <= last; k += stride) {
        ...
        const xn = k + stride <= last ? this.xOfRel(w.tsRel[k + stride]) : right;
        ctx.lineTo(xn, y);
      }
  ...
  private drawOverlayStacked(
    ...
    const vals = visible.map(({s}) => xs.map((x) => Math.max(0, this.valueAtX(s, x) ?? 0)));
    let maxTotal = 0;
    if (this.model.yRangeMode !== 'fit') {
      ...
      for (const {s} of visible) maxTotal += Math.max(0, this.ownRange(s)?.max ?? 0);
    } else {
    ...
    const range = niceValueTicks(0, maxTotal || 1, this.tickTarget(bottom - top));
    this.drawOverlayValueAxis(pal, top, bottom, range, this.overlayUnit(visible), false);
    ...
    if (focused !== undefined) {
      ...
      if (v !== undefined) {
        ...
        const y = this.model.overlayNormalize
          ...
          : this.mapY(v, this.overlayAbsRange(this.overlaySeries(), height), plotTop, plotH, log);
        ctx.beginPath();
  ...
  private overlaySeries(): ReadonlyArray<{s: CounterSeries; i: number}> {
    ...
    // ignores the filter.
    const unit =
      !this.model.overlayNormalize ? this.model.overlayUnitFilter : undefined;
    const out: Array<{s: CounterSeries; i: number}> = [];
    const sel = this.model.selected;
  ...
  private lineAt(
    ...
    if (this.model.overlayStacked) {
      ...
      const range = niceValueTicks(0, this.overlayStackMax || 1, this.tickTarget(height));
      let base = 0;
  ...
  private ownRange(series: CounterSeries): {min: number; max: number} | undefined {
    if (
  ...
  private valueRange(series: CounterSeries): {min: number; max: number} | undefined {
  ...
  private mapY(v: number, range: AxisRange, plotTop: number, plotH: number, log: boolean): number {
  ...
  private drawLane(
    ...
    ctx.strokeStyle = pal.border;
    ctx.lineWidth = 1;
    ctx.strokeRect(left + 0.5, lane.top + 0.5, right - left - 1, lane.height - 1);
    ...
    if (w !== undefined && w.count > 0) {
      ...
      ctx.fillStyle = pal.muted;
      ctx.font = AXIS_FONT;
      ctx.fillText(this.fmt(series, w.lastV[w.count - 1]), right - 6, lane.top + 13);
    }
  }
  ...
  private drawSeries(
    ...
    const nextX = (i: number) =>
      i + stride <= last ? this.xOfRel(w.tsRel[i + stride]) : this.cssW - this.padRight;

    ctx.save();
    ctx.beginPath();
    ctx.rect(this.padLeft, lane.top, this.cssW - this.padRight - this.padLeft, lane.height);
    ctx.clip();
    ...
    for (let i = 0; i <= last; i += stride) {
      ...
      ctx.rect(x, yMax, Math.max(1, xn - x), this.yOf(w.minV[i], range, lane) - yMax);
    }
  ...
  private attachListeners() {
    ...
    c.addEventListener('pointermove', (e: PointerEvent) => {
      ...
      // crosshair.
      const overShell =
        this.model.layout === 'stacked' && this.mouseX < this.padLeft && ly > TOP_AXIS;
      ...
      if (this.drag !== undefined) {
        ...
        if (this.drag.mode === 'pan') {
          ...
          const dxNs = -(dxPx / this.plotWidth()) * Number(this.drag.startView.e - this.drag.startView.s);
