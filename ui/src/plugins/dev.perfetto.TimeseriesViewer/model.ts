...
export class ViewerModel {
  ...
  setAreaSelection(ids: ReadonlyArray<number>): void {
    ...
    if (next.size === this.areaIds.size && [...next].every((id) => this.areaIds.has(id))) {
      return; // unchanged
    }
    this.areaIds.clear();
  ...
  selectedUnits(): string[] {
    ...
    for (const s of this.selected) {
      if (s.info.unit !== undefined && s.info.unit.length > 0) set.add(s.info.unit);
    }
    return [...set].sort();
  }
      ...
      for (const node of this.trace.workspaces.currentWorkspace.flatTracks) {
        ...
        if (ids) {
          for (const cid of ids) {
            if (!this.uriByCounterId.has(cid)) this.uriByCounterId.set(cid, uri);
          }
        }
      }
    }
  ...
  panBy(deltaNs: number): void {
    ...
    this.setView(Time.fromRaw(this.viewStart + d), Time.fromRaw(this.viewEnd + d));
  }
