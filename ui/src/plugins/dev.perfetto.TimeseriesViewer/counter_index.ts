...
export const COLOR_CHOICES: ReadonlyArray<string> = [
  ...
  '#46bdc6', '#9334e6', '#e91e63', '#00acc1', '#9e9e9e',
];
...
export function swatchClassFor(colorOverride: string | undefined, i: number): string {
  const ci = colorOverride ? COLOR_CHOICES.indexOf(colorOverride) : -1;
  return ci >= 0 ? `pf-tsv__color-dot--${ci}` : `pf-tsv__swatch--${(i % 8) + 1}`;
}
...
const SI_NATIVE_UNITS = new Set([
  'Hz', 'W', 'V', 'A', 'J', 'B', 'b',
  'bytes', 'bits', 'seconds', 'hertz', 'watts', 'volts', 'amps', 'joules',
]);
...
function siCompact(value: number): string {
  ...
  for (const [factor, prefix] of SI_PREFIXES) {
    if (abs >= factor) {
      ...
      const am = Math.abs(m);
      const s = am >= 100 ? m.toFixed(0) : am >= 10 ? m.toFixed(1) : m.toFixed(2);
...
export function counterUnit(rawUnit: string | undefined, name: string): string | undefined {
...
export class CounterSeries {
  ...
  private async fetchAll(): Promise<boolean> {
    ...
    this.window = {start: first, end: last, tsRel, minV: v, maxV: v, lastV: v, count};
    this.full = true;
    this.lastKey = 'full';
    return true;
  }
  ...
  async fetch(start: time, end: time, resolution: duration): Promise<boolean> {
    ...
    const cols = res.decodeColumns({ts: LONG, minV: NUM, maxV: NUM, lastV: NUM});
    const count = res.numRows();
    const tsRel = new Float64Array(count);
