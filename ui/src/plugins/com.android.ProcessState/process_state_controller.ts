...
export class ProcessStateController {
  ...
  processes: Row[] = [];
  // pid -> its _ps_process row (uid, name, ...), for enriching pids that appear
  // in other grids (bindings, hosted services/providers) with uid + name.
  private pidInfo = new Map<number, Row>();
  procCols: string[] = [];
  ...
  async setSnapshot(id: number) {
    ...
    this.pidInfo = new Map(this.processes.map((r) => [Number(r['pid']), r]));
  ...
  private async loadSelected(pid: number) {
    ...
    this.hostedSvc = this.withOwner(this.rowsOf(hsvcQ));
    this.hostedSvcCols = this.withOwnerCols(hsvcQ.columns());
    this.hostedProv = this.withOwner(this.rowsOf(hprovQ));
    this.hostedProvCols = this.withOwnerCols(hprovQ.columns());
  ...
  // Map a binding/provider summary query to grid rows, enriching the bound pid
  // with its uid and process name from the snapshot's process list.
  private bindRows(
    ...
    return this.rowsOf(q).map((b) => {
      const pid = Number(b[pidCol] ?? 0);
      const proc = this.pidInfo.get(pid);
      return {
        pid,
        uid: proc?.['uid'] ?? null,
  ...
  // Enrich rows carrying an owning_pid with the owner's uid and process name.
  private withOwner(rows: Row[]): Row[] {
    return rows.map((r) => {
      const proc = this.pidInfo.get(Number(r['owning_pid']));
      return {
        ...r,
        owning_uid: proc?.['uid'] ?? null,
        owning_process_name: proc?.['name'] ?? null,
      };
    });
  }
  // Insert owning_uid/owning_process_name right after owning_pid in a column
  // list, so they read next to the pid they describe.
  private withOwnerCols(cols: string[]): string[] {
    const i = cols.indexOf('owning_pid');
    if (i < 0) return cols;
    return [
      ...cols.slice(0, i + 1),
      'owning_uid',
  ...
  private buildEdge(e: EdgeSel) {
    const client = this.pidInfo.get(e.from);
    const host = this.pidInfo.get(e.to);
    this.edgeRows = [
      {
        client_pid: e.from,
        client_uid: client?.['uid'] ?? null,
        client_process_name: client?.['name'] ?? null,
        host_pid: e.to,
        host_uid: host?.['uid'] ?? null,
        host_process_name: host?.['name'] ?? null,
        connections: e.count,
