...
import {type EdgeSel, sameEdge} from './process_graph';
  ...
  implements m.ClassComponent<ProcessStatePageAttrs>
{
  ...
  // Self bindings: this process bound to its OWN hosted services/providers.
  // Kept out of in/outbound (those are cross-process) but surfaced separately.
  private selfRows: Row[] = [];
  private provSelfRows: Row[] = [];
  ...
  private selfDs?: InMemoryDataSource;
  ...
  private async loadSelected() {
    ...
    if (snap === undefined || this.selectedPid === undefined) {
      ...
      this.selfRows = this.provSelfRows = this.selfAll = [];
    ...
    // Self bindings: this pid is a client of a service/provider it also hosts.
    const selfSvcQ = await this.trace.engine.query(`
      SELECT s.short_name AS service,
             MAX(b.flag_foreground_service) AS fg, COUNT(*) AS n
      FROM _ps_binding b
      JOIN _ps_service s
        ON s.snapshot_id = b.snapshot_id AND s.service_id = b.service_id
    ...
    this.selfRows = this.rowsOf(selfSvcQ);
    this.provSelfRows = this.rowsOf(selfProvQ);
  ...
  private buildDetailGrids(pid: number) {
    ...
    // Self bindings: own pid is implicit, so just kind / name / fg / count.
    this.selfAll = [
      ...this.selfRows.map((b) => ({
        kind: 'service',
        name: String(b['service'] ?? ''),
  ...
  private selectEdge(e: EdgeSel) {
    // Clicking the already-selected edge again clears it (toggle off).
    ...
    return m('.pf-ps-detailpane', [
      ...
      m('.pf-ps-cards', [
        m('.pf-ps-col', [
          ...
          // Only when the source actually has uid records (the dumpsys path
          // has none); empty, it adds no value.
          this.uidRow.length > 0 &&
            this.recordCard(
              'uid record',
              'uid',
              this.uidCols,
          ...
          // Hidden when there are none (e.g. the dumpsys path has no providers).
          this.hostedProv.length > 0 &&
            this.recordCard(
              'hosted providers',
              'hostedProv',
              this.hostedProvCols,
        ...
        m('.pf-ps-col', [
          ...
          this.selfAll.length > 0 &&
            this.gridCard(
              'self bindings \u2014 binds its own hosted components',
              ['kind', 'name', 'fg', 'n'],
              this.selfAll,
              this.selfDs,
            ),
