...
import {
  procStateName, schedGroupName, capabilityNames, fgsTypeNames, hostingTypeNames,
} from './enums';
...
const DEFAULT_PROC_COLS = [
  'pid', 'name', 'uid', 'cur_adj', 'adj_type', 'cur_proc_state',
  'cur_sched_group', 'cur_capability', 'is_frozen', 'persistent', 'cached_adj',
  'adj_source_pid',
];
...
const DEFAULT_BINDING_COLS = [
  'client_pid', 'service', 'flag_foreground_service', 'flag_above_client',
  'flag_include_capabilities', 'flag_important', 'effective_proc_state',
  'client_uid',
];
const DEFAULT_PROVBIND_COLS = [
  'client_pid', 'authority', 'stable_count', 'unstable_count', 'dead', 'waiting',
];
const DEFAULT_SERVICE_COLS = [
  'short_name', 'is_foreground', 'foreground_service_type', 'start_requested',
  'is_short_fgs', 'execute_nesting', 'restart_count', 'crash_count',
];
const DEFAULT_PROVIDER_COLS = [
  'authority', 'package_name', 'external_handle_count', 'launched',
];
const DEFAULT_UID_COLS = [
  'uid', 'cur_proc_state', 'cur_capability', 'idle', 'restriction_level',
  'standby_bucket',
];
...
export class ProcessStatePage implements m.ClassComponent<ProcessStatePageAttrs> {
  ...
  private async loadSnapshot() {
    ...
    if (this.selectedPids.size > 0) {
      ...
      this.syncPrimary();
      if (this.selectedPids.size === 0 && this.tab === 'detail'
  ...
  private buildDetailGrids(pid: number) {
    ...
    this.oomAdjRows = [
      ...
      {property: 'proc state', value: p ? procStateName(p['cur_proc_state']) : '—'},
      {property: 'sched group', value: p ? schedGroupName(p['cur_sched_group']) : '—'},
      {property: 'capability', value: p ? capabilityNames(p['cur_capability']) : '—'},
      {property: 'fg service types',
        value: p ? fgsTypeNames(p['fg_service_types']) : '—'},
      {property: 'hosting', value: p ? hostingTypeNames(p['hosting_component_types']) : '—'},
    ...
    this.outAll = [
      ...
      ...this.provOutRows.map((b) => ({
        ...
        name: String(b['authority'] ?? ''), fg: '', n: Number(b['n'] ?? 1),
      })),
    ];
    this.inAll = [
      ...this.inRows.map((b) => ({
  ...
  private syncPrimary() {
    this.selectedPid = this.selectedPids.size === 1
      ? [...this.selectedPids][0] : undefined;
  }
  ...
  private selectEdge(e: EdgeSel) {
    ...
    this.tab = 'detail';
    this.loadEdge(e).then(() => m.redraw()).catch((err) => console.error(err));
  }
  ...
  private async loadEdge(e: EdgeSel) {
    const snap = this.snapshots[this.idx];
    if (snap === undefined) {
      this.edgeRows = []; this.edgeCols = []; this.edgeDs = undefined; return;
    }
    const q = e.kind === 'provider'
  ...
  view() {
    ...
    const snap = this.snapshots[this.idx];
    const hasSel = this.selectedPids.size > 0 || this.selectedEdge !== undefined;
    return m('.pf-ps-page', [
      this.renderSeek(snap),
      this.procDs &&
        m('.pf-ps-graphwrap', {style: `flex: 0 0 ${this.graphPx}px`},
          m(ProcessGraph, {
      ...
      m('.pf-ps-resize', {
        onpointerdown: (e: PointerEvent) => {
          this.resizing = true; this.rsY = e.clientY; this.rsPx = this.graphPx;
        ...
        onpointermove: (e: PointerEvent) => {
          if (this.resizing) {
            this.graphPx = Math.min(this.rsMax,
              Math.max(140, this.rsPx + (e.clientY - this.rsY)));
          }
        },
        onpointerup: () => {this.resizing = false;},
      }),
      m('.pf-ps-bottom', [
        m('.pf-ps-tabs', [
          ...
          hasSel
            ? this.tabBtn('detail',
                this.selectedEdge ? '▸ binding'
                  : this.selectedPids.size > 1 ? `▸ ${this.selectedPids.size} selected`
  ...
  private tabBody(): m.Children {
    ...
    const visible = this.procColumns
    ...
    return this.procDs
      ? m(DataGrid, {
          ...
          columns: visible,
          onColumnsChanged: (cols) => {this.procColumns = cols;},
        })
  ...
  private renderMultiDetail(): m.Children {
    const rows = this.procRows.filter((r) => this.selectedPids.has(Number(r['pid'])));
    const ds = new InMemoryDataSource(rows);
    return m('.pf-ps-detailpane', [
      m('.pf-ps-detail-h', [
        m('span.pf-ps-detail-title', `${this.selectedPids.size} processes selected`),
        m('span.pf-ps-detail-sub', 'shift-click a node to add/remove · click empty space to clear'),
      ]),
      this.gridCard('selection',
        DEFAULT_PROC_COLS.filter((c) => this.procCols.includes(c)), rows, ds),
    ]);
  }
  ...
  private recordCard(title: string, key: string, allCols: string[],
      defaultCols: string[], rows: Row[], ds?: InMemoryDataSource): m.Children {
    ...
    const visible = this.recordColumns[key]
      ?? defaultCols.filter((c) => allCols.includes(c)).map((c) => ({id: c, field: c}));
    return m('.pf-ps-card', [
      m('.pf-ps-card-h', title),
      m(DataGrid, {
        schema: gridSchema(allCols, (pid) => this.select(pid)),
        rootSchema: 'root', data: ds, columns: visible,
        onColumnsChanged: (cols) => {this.recordColumns[key] = cols;},
      }),
    ]);
  }
  ...
  private renderProcessDetail(pid: number): m.Children {
    ...
    return m('.pf-ps-detailpane', [
      ...
      m('.pf-ps-cards', [
        m('.pf-ps-col', [
          this.gridCard('process state', ['property', 'value'], this.oomAdjRows, this.oomAdjDs),
          this.recordCard('uid record', 'uid', this.uidCols, DEFAULT_UID_COLS,
            this.uidRow, this.uidDs),
          this.recordCard('hosted services', 'hostedSvc', this.hostedSvcCols,
            DEFAULT_SERVICE_COLS, this.hostedSvc, this.hostedSvcDs),
          this.recordCard('hosted providers', 'hostedProv', this.hostedProvCols,
            DEFAULT_PROVIDER_COLS, this.hostedProv, this.hostedProvDs),
        ]),
        m('.pf-ps-col', [
          this.gridCard('depends on \u2014 outbound',
            ['pid', 'kind', 'name', 'fg', 'n'], this.outAll, this.outDs),
          this.gridCard('depended on by \u2014 inbound',
            ['pid', 'kind', 'name', 'fg', 'n'], this.inAll, this.inDs),
        ]),
      ]),
    ]);
  }
  ...
  private renderEdgeDetail(e: EdgeSel): m.Children {
    ...
    const visible = this.edgeColumns
      ?? def.filter((c) => this.edgeCols.includes(c)).map((c) => ({id: c, field: c}));
    const nm = this.nameOf(e.from);
    const host = this.nameOf(e.to);
    return m('.pf-ps-detailpane', [
      m('.pf-ps-detail-h', [
        m('span.pf-ps-detail-title',
          `${provider ? 'content-provider' : 'service'} binding`
          + ` · ${this.edgeRows.length}`),
        m('span.pf-ps-detail-sub',
          `${nm} (${e.from}) → ${host} (${e.to}) · every column addable`),
      ]),
      m('.pf-ps-card', [
        m('.pf-ps-card-h', provider ? 'content-provider binding record'
        ...
        this.edgeRows.length && this.edgeDs
          ? m(DataGrid, {
              ...
              rootSchema: 'root', data: this.edgeDs, columns: visible,
              onColumnsChanged: (cols) => {this.edgeColumns = cols;},
            })
          : m('.pf-ps-card-b', m('.pf-ps-none', '— none —')),
      ]),
    ]);
