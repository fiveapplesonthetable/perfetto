...
export interface ProcessGraphAttrs {
  ...
  // The selected edge, owned by the host (like selectedPids) so selection
  // state — including toggle-off and clearing on node/empty clicks — lives in
  // one place and the graph just renders it.
  ...
  view({attrs}: m.Vnode<ProcessGraphAttrs>) {
    ...
    const focusPids =
      this.hoverPid !== undefined
        ...
        : new Set<number>(sel ?? []);
    if (attrs.selectedEdge) focusPids.add(attrs.selectedEdge.from);
