...
const TIER_LABEL = [
  'persistent', 'foreground & visible', 'perceptible', 'service',
  'background', 'cached',
];
...
export class ProcessGraph implements m.ClassComponent<ProcessGraphAttrs> {
  ...
  private edgeTooltip(e: Edge, byPid: Map<number, Row>): m.Children {
    ...
    const kind = e.kind === 'provider' ? 'content-provider binding'
      : e.fg ? 'foreground service binding' : 'service binding';
    return [
  ...
  view({attrs}: m.Vnode<ProcessGraphAttrs>) {
    ...
    const focusPids = this.hoverPid !== undefined
    ...
    const bold = this.hoverPid === undefined || (sel?.has(this.hoverPid) ?? false);
    ...
    if (focusPids.size > 0) {
      ...
      for (const e of this.edges) {
        ...
        const ax = a.x + radiusOf(e.from); const bx = b.x - radiusOf(e.to);
        const mx = (ax + bx) / 2;
        const d = `M ${ax} ${a.y} C ${mx} ${a.y}, ${mx} ${b.y}, ${bx} ${b.y}`;
        edgeEls.push(m('g.pf-ps-edge', {
          onmouseenter: () => {this.hoverEdge = e;},
          onmouseleave: () => {if (this.hoverEdge === e) this.hoverEdge = undefined;},
    ...
    const baseEls: m.Children[] = [];
    {
      for (const e of this.edges) {
        ...
        baseEls.push(m('g.pf-ps-edge', {
          ...
          m('path', {d, "fill": 'none', "stroke": 'transparent', 'stroke-width': 7}),
          m('path', {
            d, "fill": 'none',
            "stroke": e.kind === 'provider' ? EDGE_PROVIDER : e.fg ? EDGE_FG : EDGE_SERVICE,
    ...
    for (const [pid, p] of pos) {
      ...
      const dim = focusPids.size > 0 && !focusPids.has(pid) && !neighbours.has(pid);
      // Chop hard (full name is in the hover tooltip); show … when truncated.
      const shortName = p.name.replace(/^.*\//, '');
      const label =
        `${shortName.length > 15 ? shortName.slice(0, 14) + '…' : shortName} ${pid}`;
      const nodeR = radiusOf(pid);
      nodeEls.push(
        m('g.pf-ps-node' + (isSel ? '.pf-ps-node--sel' : ''), {
          onclick: (ev: MouseEvent) => {
            ev.stopPropagation();
          ...
          m('circle', {
            cx: p.x, cy: p.y, r: isSel ? nodeR + 2 : nodeR,
    ...
    const headers = activeTiers.map((t, ci) =>
      m('text.pf-ps-col',
        {"x": left + ci * colW, "y": 18, 'font-size': 11, 'font-weight': 'bold'},
        TIER_LABEL[t]));
    ...
    if (!this.userAdjusted || this.vb === undefined) {
      ...
      if (this.vw > 0 && this.vh > 0) {
        ...
        if (w / h < ar) h = w / ar; // wide viewport: fill width, pan vertically
    ...
    const tip = this.dragging || !this.sawPointer
      ? undefined
      : this.hoverEdge
    ...
    return m('.pf-ps-graph', [
      ...
      m('svg.pf-ps-svg', {
        ...
        onupdate: (v: m.VnodeDOM) => {
          ...
          if (r.width > 0
              && (Math.abs(r.width - this.vw) > 1 || Math.abs(r.height - this.vh) > 1)) {
            this.vw = r.width; this.vh = r.height;
        ...
        m('defs',
          m('marker', {
            ...
            markerWidth: 1.6, markerHeight: 1.6, orient: 'auto-start-reverse',
