...
export class ProcessStateDetailsPanel implements TrackEventDetailsPanel {
  ...
  render() {
    ...
    return m(
      ...
      m('.pf-ps-panelsplit', {
        oncreate: (v: m.VnodeDOM) => {this.splitW = v.dom.clientWidth;},
        onupdate: (v: m.VnodeDOM) => {this.splitW = v.dom.clientWidth;},
      }, [
        m('.pf-ps-detailgraph', {style: `flex: 0 0 ${(this.graphFrac * 100).toFixed(1)}%`},
          m(ProcessGraph, {
            trace: this.trace,
            processes: this.processes,
            bindingsQuery: id,
            selectedPids: this.selectedPid !== undefined
              ? new Set([this.selectedPid]) : undefined,
            ...
            onDeselect: () => {
              this.selectedPid = undefined; this.selectedEdge = undefined; m.redraw();
            },
          })),
        m('.pf-ps-vresize', {
          onpointerdown: (e: PointerEvent) => {
            this.dragging = true; this.dragX = e.clientX; this.dragFrac = this.graphFrac;
            (e.currentTarget as Element).setPointerCapture(e.pointerId);
          },
          onpointermove: (e: PointerEvent) => {
            if (this.dragging && this.splitW > 0) {
              const f = this.dragFrac + (e.clientX - this.dragX) / this.splitW;
  ...
  private renderProps(): m.Children {
    ...
    if (this.selectedPid !== undefined) {
      return m('.pf-ps-detailpane', [
        ...
        gridCard('process state', ['property', 'value'], this.nodeRows, this.nodeDs, onPid),
        m('.pf-ps-none', 'Open the full explorer for how-this-adj-was-computed & bindings.'),
      ]);
    }
