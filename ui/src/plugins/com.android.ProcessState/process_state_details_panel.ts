...
export class ProcessStateDetailsPanel implements TrackEventDetailsPanel {
  ...
  async load(sel: TrackEventSelection) {
    ...
    const meta = await this.trace.engine.query(`
      ...
      FROM _ps_snapshot WHERE id = ${sel.eventId}`);
        ...
        [
          m(
            ...
            m(ProcessGraph, {
              ...
              onSelect: (pid: number) => {
                // Re-clicking the selected node clears it (toggle off).
              ...
              onEdgeSelect: (e) => {
                // Re-clicking the selected edge clears it (toggle off).
                if (
                  this.selectedEdge !== undefined &&
                  sameEdge(this.selectedEdge, e)
                ) {
                  this.selectedEdge = undefined;
                  m.redraw();
                  return;
