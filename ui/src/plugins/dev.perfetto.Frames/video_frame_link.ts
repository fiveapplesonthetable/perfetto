...
// single join on the video frame's matched vsync id covers both. Renders
// nothing when there's no matching frame, so it self-hides on traces without
// video.
  ...
  async load(selection: TrackEventSelection): Promise<void> {
    ...
    try {
      ...
      const res = await this.trace.engine.query(`
        SELECT dv.id AS id, dv.display_id AS displayId
        FROM actual_frame_timeline_slice s
        JOIN android_display_video_frames dv
          ON dv.vsync_id = s.display_frame_token
